...
module weight_buffer_array #(
    ...
    // BRAM read address를 증가시키는 구간
    // lenet5_top.v에서는 이 신호로 w_addr_r도 증가함
    input wire shift_en,
    ...
    // 핵심 수정:
    // BRAM read latency 보정을 위해 shift_en을 1클럭 delay해서 capture_en으로 사용
    reg shift_en_d;

    integer i;

    wire capture_en = shift_en_d;
    ...
    always @(posedge clk or negedge reset_n) begin
        if (!reset_n) begin
            ...
            //   w_addr_r 증가용
            //
            // shift_en_d / capture_en:
            //   BRAM latency 1클럭 보정 후 실제 buffer 저장용
            // -----------------------------------------------------
            shift_en_d <= shift_en;
            ...
            //
            // shift_en이 5클럭 켜졌다면:
            //   capture_en은 그 다음 클럭까지 포함해서 5번 유효 저장됨
            //
            // 예:
            //   bram_w_0 실제 흐름이 fd, 00, fb, fe, 01이면
            //   buf_0[0] = fd
            //   buf_0[1] = 00
            //   buf_0[2] = fb
            //   buf_0[3] = fe
            ...
            //
            // load_en은 opcode == 1일 때 들어옴.
            // top_input_router는 opcode == 1일 때만 pe_w_*를 top_in으로 보내고,
            // opcode == 2일 때는 bias/psum을 선택하므로 이쪽 경로와 충돌하지 않음.
            // -----------------------------------------------------
            if (load_en) begin
                if (load_idx == 3'd4)
                    load_idx <= 3'd0;
