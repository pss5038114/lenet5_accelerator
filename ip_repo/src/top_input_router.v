`timescale 1ns / 1ps

module top_input_router #(
    parameter BW_W = 8,
    parameter BW_P = 32
)(
    // opcode:
    // 0 = IDLE
    // 1 = PRELOAD / LOAD weight
    // 2 = CALC / MAC
    input wire [1:0] opcode,
    input wire use_bias,

    // Weight BRAM에서 바로 들어오는 8-bit weight
    input wire [BW_W-1:0] w_0, w_1, w_2, w_3,
    input wire [BW_W-1:0] w_4, w_5, w_6, w_7,

    // Bias BRAM에서 오는 32-bit bias
    input wire [BW_P-1:0] b_0, b_1, b_2, b_3,
    input wire [BW_P-1:0] b_4, b_5, b_6, b_7,

    // Output BRAM에서 오는 partial sum
    input wire [BW_P-1:0] p_0, p_1, p_2, p_3,
    input wire [BW_P-1:0] p_4, p_5, p_6, p_7,

    // PE Array top input
    output reg [BW_P-1:0] top_in_0, top_in_1, top_in_2, top_in_3,
    output reg [BW_P-1:0] top_in_4, top_in_5, top_in_6, top_in_7
);

    always @(*) begin
        if (opcode == 2'd1) begin
            // PRELOAD:
            // PE는 top_in[7:0]만 weight_reg에 저장하므로
            // 상위 24비트는 0으로 둬도 됨.
            top_in_0 = {24'd0, w_0};
            top_in_1 = {24'd0, w_1};
            top_in_2 = {24'd0, w_2};
            top_in_3 = {24'd0, w_3};
            top_in_4 = {24'd0, w_4};
            top_in_5 = {24'd0, w_5};
            top_in_6 = {24'd0, w_6};
            top_in_7 = {24'd0, w_7};
        end
        else if (opcode == 2'd2) begin
            // CALC:
            // 첫 pass에서는 bias,
            // 이후 pass에서는 output_bram의 partial sum 사용
            top_in_0 = use_bias ? b_0 : p_0;
            top_in_1 = use_bias ? b_1 : p_1;
            top_in_2 = use_bias ? b_2 : p_2;
            top_in_3 = use_bias ? b_3 : p_3;
            top_in_4 = use_bias ? b_4 : p_4;
            top_in_5 = use_bias ? b_5 : p_5;
            top_in_6 = use_bias ? b_6 : p_6;
            top_in_7 = use_bias ? b_7 : p_7;
        end
        else begin
            top_in_0 = 0;
            top_in_1 = 0;
            top_in_2 = 0;
            top_in_3 = 0;
            top_in_4 = 0;
            top_in_5 = 0;
            top_in_6 = 0;
            top_in_7 = 0;
        end
    end

endmodule